...
namespace AncientRimShields
{
    ...
    public static class HarmonyPatches
    {
        ...
        private static readonly MethodInfo statOffsetFromGearMethod =
            AccessTools.Method(typeof(StatWorker), "StatOffsetFromGear");
        ...
        {
            var harmony = new Harmony("Mlie.AncientRimShields");
            ...
            harmony.Patch(
                ...
                postfix: new HarmonyMethod(typeof(HarmonyPatches),
                    nameof(Patch_Pawn_HealthTracker_CheckForStateChance)));
            harmony.Patch(AccessTools.Method(typeof(Pawn_HealthTracker), nameof(Pawn_HealthTracker.PreApplyDamage)),
                new HarmonyMethod(typeof(HarmonyPatches), nameof(Patch_Pawn_HealthTracker_PreApplyDamage)));
            ...
            harmony.Patch(AccessTools.Method(typeof(StatWorker), nameof(StatWorker.GetValueUnfinalized)),
                transpiler: new HarmonyMethod(typeof(HarmonyPatches),
                    nameof(Transpiler_StatWorker_GetValueUnfinalized)));
            harmony.Patch(AccessTools.Method(typeof(StatWorker), nameof(StatWorker.GetExplanationUnfinalized)),
                transpiler: new HarmonyMethod(typeof(HarmonyPatches),
                    nameof(Transpiler_StatWorker_GetExplanationUnfinalized)));
        }
        ...
        public static IEnumerable<CodeInstruction> Transpiler_StatWorker_GetValueUnfinalized(
            ...
            MethodBase method, ILGenerator ilGen)
        {
            ...
            instructionList.SafeInsertRange(insertionIndex, new[]
            {
                ...
                new CodeInstruction(OpCodes.Call,
                    AccessTools.Method(typeof(HarmonyPatches), nameof(StatWorkerInjection_AddShieldValue)))
            }, labelsToTransfer);

            return instructionList;
        }

        private static void StatWorkerInjection_AddShieldValue(ref float result, Pawn_EquipmentTracker equipment,
            StatDef stat)
        {
            ...
            MethodBase method, ILGenerator ilGen)
        {
            ...
            instructionList.SafeInsertRange(insertionIndex, new[]
            {
                ...
                new CodeInstruction(OpCodes.Ldfld, statWorkerStatField), // ...stat
                new CodeInstruction(OpCodes.Call,
                    AccessTools.Method(typeof(HarmonyPatches), nameof(StatWorkerInjection_BuildShieldString)))
            }, labelsToTransfer);

            return instructionList;
        }

        private static void StatWorkerInjection_BuildShieldString(StringBuilder stringBuilder,
        ...
        public static void Patch_Pawn_EquipmentTracker_MakeRoomFor(Pawn_EquipmentTracker __instance, Pawn ___pawn,
            ThingWithComps eq)
        {
            var shieldComp = eq.GetComp<CompShield>();
            if (shieldComp == null)
            {
                return;
            }
        ...
        public static bool Patch_Pawn_HealthTracker_PreApplyDamage(Pawn ___pawn, ref DamageInfo dinfo,
            out bool absorbed)
        {
            absorbed = false;
